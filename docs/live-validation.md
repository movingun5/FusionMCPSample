...
- Live atomic parameter-batch status: **passed on Fusion 2704.1.53 with server 2.5.0**
...
## Live server 2.5.0 parameter-batch evidence

After restarting Fusion 2704.1.53 with the installed server 2.5.0 add-in, the authenticated catalog exposed all 24 tools including `update_parameter_batch`. Against a new unsaved blank Part Design, the explicit-tool harness completed 10 of 10 checks:

1. Create `BatchPlate` at `40 × 30 × 4 mm`, driven by `batch_plate_width`, `batch_plate_height`, and `batch_plate_thickness`. The centered bounds were `[-20, -15, 0]` to `[20, 15, 4]` and the volume was `4.8 cm³`.
2. Submit a stale width expectation of `39 mm` and confirm the structured `PARAMETER_CONFLICT` refusal left all parameters and geometry unchanged.
3. Atomically update width from `40 mm` to `60 mm` and thickness from `4 mm` to `6 mm`. Fusion performed one recompute and checkpoint, retained centered bounds of `[-30, -15, 0]` to `[30, 15, 6]`, and produced the expected `60 × 30 × 6 mm` body with volume `10.8 cm³`.
4. Call `undo_last_execution` once and confirm both expressions and the whole body returned to `40 × 30 × 4 mm`, centered at `[-20, -15, 0]` to `[20, 15, 4]`, with volume `4.8 cm³`.
